...
function Comments(props) {
    ...
    const getComments = async () => {
        ...
        let getComments = axios({
          method: "post",
          url: `${globals.productionServerDomain}/getCourseCommentsWithCourseId`,
          data: data,
        })
          .then(function (res) {
            if (res.data[0]){
  ...
  return (  
    <div style={{display: comments.length > 0 ? "block" : "none"}} className={styles.wrapper}>
        <h1 className={styles.title}>Отзывы учеников</h1>
    <div className={styles.commentsWrapper}>  
        {comments?.map(el => 
            <CourseComment comment={el}/>
        )}
